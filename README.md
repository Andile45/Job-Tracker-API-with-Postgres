...
4. Set up the database  
   ...
   - Run the SQL schema below to create tables.  
5. Start the server  
   ```bash
   ...
   npm run dev  
   ```  
6. The API should now be running at `http://localhost:PORT` (default `PORT=5000`).
...
- Deleting a user removes all their applications automatically (`ON DELETE CASCADE`).  
